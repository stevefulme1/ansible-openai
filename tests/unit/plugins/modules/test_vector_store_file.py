"""Unit tests for stevefulme1.openai.vector_store_file module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.openai.plugins.modules.vector_store_file"
CLIENT_PATH = "ansible_collections.stevefulme1.openai.plugins.module_utils.openai_client"


def _build_resource(**overrides):
    """Return a mock vector_store_file resource dict."""
    base = {
        "id": "res-123",
...
@pytest.fixture
def module_args():
    """Base module args for vector_store_file."""
    return {
        "state": "present",
        "api_key": "sk-test-key",
        "organization": None,
        ...
        "timeout": 30,
        "vector_store_id": "test-vector_store_id",
        "file_id": "test-file_id"
    }


class TestCreate:
    """Test vector_store_file creation."""
    ...
    def test_create_resource(self, mock_ansible_cls, mock_client_cls, module_args):
        ...
        from ansible_collections.stevefulme1.openai.plugins.modules.vector_store_file import main
        try:
            main()
        except SystemExit:
            pass
...
class TestDelete:
    """Test vector_store_file deletion."""
    ...
    def test_delete_resource(self, mock_ansible_cls, mock_client_cls, module_args):
        ...
        module_args["vector_store_id"] = "res-123"
        mock_module = MagicMock()
        mock_module.exit_json = MagicMock(side_effect=SystemExit(0))
        mock_module.fail_json = MagicMock(side_effect=SystemExit(1))
        mock_module.params = module_args
...
class TestUpdate:
    """Test vector_store_file update with existing ID."""
