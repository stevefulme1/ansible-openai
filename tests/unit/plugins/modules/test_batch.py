...
def _build_resource(**overrides):
    """Return a mock batch resource dict."""
    base = {
        "id": "res-123",
        "completion_window": "test-completion_window",
        "endpoint": "test-endpoint",
        "input_file_id": "test-input_file_id",
...
def resource_args(module_args):
    """Module args for batch operations."""
    module_args.update({
        "state": "present",
...
class TestGetCurrentState:
    ...
    def test_returns_matching_resource(self, resource_args):
        ...
        from ansible_collections.stevefulme1.openai.plugins.modules.batch import get_current_state
...
class TestNeedsUpdate:
    ...
    def test_returns_true_when_no_current(self):
        ...
        from ansible_collections.stevefulme1.openai.plugins.modules.batch import needs_update
...
class TestBuildPayload:
    ...
    def test_builds_payload_from_params(self, resource_args):
        ...
        from ansible_collections.stevefulme1.openai.plugins.modules.batch import build_payload
...
class TestCreate:
    ...
    def test_create_sets_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        with patch(f"{MODULE_PATH}.get_current_state", return_value=None):
            from ansible_collections.stevefulme1.openai.plugins.modules.batch import main
            main()
...
class TestUpdate:
    ...
    def test_update_when_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        resource_args["completion_window"] = "new-value"
        mock_module = MagicMock()
        mock_module.params = resource_args
        mock_module.check_mode = False
        mock_ansible_cls.return_value = mock_module
        ...
        mock_client = MagicMock()
        mock_client.put.return_value = _build_resource(completion_window="new-value")
        mock_client_cls.return_value = mock_client

        existing = _build_resource(completion_window="old-value")
        with patch(f"{MODULE_PATH}.get_current_state", return_value=existing), \
