"""Unit tests for stevefulme1.openai.batch module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.openai.plugins.modules.batch"
CLIENT_PATH = "ansible_collections.stevefulme1.openai.plugins.module_utils.openai_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for batch."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"batch_id": "res-123", "endpoint": "test-batch"}
    client.update.return_value = {"batch_id": "res-123", "endpoint": "test-batch-updated"}
...
def existing_resource():
    """Return a dict representing an existing batch."""
    return {
        "batch_id": "res-123",
        "endpoint": "test-batch",
        "state": "active",
...
class TestCreateBatch:
    """Tests for creating a batch."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("batch", {"endpoint": "test-batch"})
        assert result["batch_id"] == "res-123"
        assert result["endpoint"] == "test-batch"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("batch", params)
        mock_api_client.create.assert_called_once_with("batch", params)
...
class TestUpdateBatch:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("batch", "res-123", {"endpoint": "updated"})
        assert result["endpoint"] == "test-batch-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"endpoint": existing_resource["endpoint"]}
        current = {"endpoint": existing_resource["endpoint"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("batch", "bad-id", {})


class TestDeleteBatch:
    """Tests for deleting a batch."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("batch", "res-123")
        mock_api_client.delete.assert_called_once_with("batch", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("batch", "missing-id")
...
class TestGetBatch:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("batch", "res-123")


class TestListBatch:
    """Tests for listing batch resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"batch_id": "1", "endpoint": "first"},
            {"batch_id": "2", "endpoint": "second"},
        ]
        result = mock_api_client.list("batch")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"batch_id": "1", "endpoint": "match"}]
        result = mock_api_client.list("batch", filters={"endpoint": "match"})
        assert len(result) == 1


class TestIdempotencyBatch:
    """Tests for idempotent behavior of batch."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("batch", "res-123")
        desired_params = {"endpoint": current["endpoint"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["endpoint"] != current["endpoint"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("batch", "missing") is not None
