...
def _build_resource(**overrides):
    """Return a mock fine_tuning_job resource dict."""
    base = {
        "id": "res-123",
        "model": "test-model",
        "training_file": "test-training_file",
        "hyperparameters": "test-hyperparameters",
...
def resource_args(module_args):
    """Module args for fine_tuning_job operations."""
    module_args.update({
        "state": "present",
...
class TestGetCurrentState:
    ...
    def test_returns_matching_resource(self, resource_args):
        ...
        from ansible_collections.stevefulme1.openai.plugins.modules.fine_tuning_job import get_current_state
...
class TestNeedsUpdate:
    ...
    def test_returns_true_when_no_current(self):
        ...
        from ansible_collections.stevefulme1.openai.plugins.modules.fine_tuning_job import needs_update
...
class TestBuildPayload:
    ...
    def test_builds_payload_from_params(self, resource_args):
        ...
        from ansible_collections.stevefulme1.openai.plugins.modules.fine_tuning_job import build_payload
...
class TestCreate:
    ...
    def test_create_sets_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        with patch(f"{MODULE_PATH}.get_current_state", return_value=None):
            from ansible_collections.stevefulme1.openai.plugins.modules.fine_tuning_job import main
            main()
...
class TestUpdate:
    ...
    def test_update_when_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        mock_client = MagicMock()
        mock_client.put.return_value = _build_resource(model="new-value")
        mock_client_cls.return_value = mock_client

        existing = _build_resource(model="old-value")
        with patch(f"{MODULE_PATH}.get_current_state", return_value=existing), \
