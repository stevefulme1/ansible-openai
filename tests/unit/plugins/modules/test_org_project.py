"""Unit tests for stevefulme1.openai.org_project module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.openai.plugins.modules.org_project"
CLIENT_PATH = "ansible_collections.stevefulme1.openai.plugins.module_utils.openai_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for org_project."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"project_id": "res-123", "name": "test-org_project"}
    client.update.return_value = {"project_id": "res-123", "name": "test-org_project-updated"}
...
def existing_resource():
    """Return a dict representing an existing org_project."""
    return {
        "project_id": "res-123",
        "name": "test-org_project",
        "state": "active",
    }


class TestCreateOrgProject:
    """Tests for creating a org_project."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("org_project", {"name": "test-org_project"})
        assert result["project_id"] == "res-123"
        assert result["name"] == "test-org_project"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "name": "full-org_project",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("org_project", params)
        mock_api_client.create.assert_called_once_with("org_project", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("org_project", {"name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "org_project": {}}
        else:
            result = mock_api_client.create("org_project", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateOrgProject:
    """Tests for updating a org_project."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("org_project", "res-123", {"name": "updated"})
        assert result["name"] == "test-org_project-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("org_project", "bad-id", {})


class TestDeleteOrgProject:
    """Tests for deleting a org_project."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("org_project", "res-123")
        mock_api_client.delete.assert_called_once_with("org_project", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("org_project", "missing-id")
...
class TestGetOrgProject:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("org_project", "res-123")


class TestListOrgProject:
    """Tests for listing org_project resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"project_id": "1", "name": "first"},
            {"project_id": "2", "name": "second"},
        ]
        result = mock_api_client.list("org_project")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"project_id": "1", "name": "match"}]
        result = mock_api_client.list("org_project", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyOrgProject:
    """Tests for idempotent behavior of org_project."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("org_project", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("org_project", "missing") is not None
        assert exists is False


class TestErrorHandlingOrgProject:
    """Tests for error handling in org_project."""
